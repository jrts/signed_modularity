...
def modularity_louvain_und_sign(W, gamma=1, qtype='sta', seed=None):
    ...
    while q[h] - q[h - 1] > 1e-10:
        if h > 300:
            raise Exception('Modularity Infinite Loop Style A.  Please '
        ...
        while flag:
            it += 1
            if it > 1000:
                raise Exception('Infinite Loop was detected and stopped. '
...
def main():
    ...
    ci_ret, q = modularity_louvain_und_sign(am, qtype='neg', seed=5)
    print(ci_ret)
    print(q)
